...
void add_to_hist() {
  if (hist_pos == HIST_CAP - 1) {
    memmove(&hist[0], &hist[1], HIST_CAP - 1);
    hist_pos -= 1;
  }
  hist_pos += 1;
  hist_len = hist_pos + 1;
  hist[hist_pos].x = mouseinfo.x;
  hist[hist_pos].y = mouseinfo.y;
}
...
void handle_key(KeyCode keycode, Bool is_press) {
    ...
    // TODO: Make configurable in config.h
    if (keysym == XK_z) {
      speed *= 1.75;
    }
    if (keysym == XK_g) {
    ...
    if (keysym == XK_u && is_press) {
      if (hist_pos > 0) {
        hist_pos -= 1;
        XWarpPointer(dpy, None, root, 0, 0, 0, 0, hist[hist_pos].x, hist[hist_pos].y);
        XFlush(dpy);
      }
    }
    if (keysym == XK_r && is_press) {
      if (hist_pos < HIST_CAP - 1) {
        hist_pos += 1;
        XWarpPointer(dpy, None, root, 0, 0, 0, 0, hist[hist_pos].x, hist[hist_pos].y);
        XFlush(dpy);
